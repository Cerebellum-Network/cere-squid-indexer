...
/**
 * Log warning when using default cluster ID for legacy events
 * @param blockHeight - The block height where the event occurred
 * @param eventType - The type of event (e.g., 'deposited', 'charged')
 */
export function logLegacyEventClusterIdWarning(blockHeight: number, eventType: string): void {
    console.warn(`Using default cluster ID ${DEFAULT_CLUSTER_ID} for legacy ${eventType} event at block ${blockHeight}`)
}
